...
```go
// 使用 pkg/encrypt 包进行密码加密
import "github.com/clin211/miniblog-v3/pkg/encrypt"
...
// 验证密码
func (l *UserLogic) verifyPassword(password, hashedPassword string) error {
    return encrypt.Compare(hashedPassword, password)
}
```
...
```go
// 生成 JWT Token
func (l *UserLogic) generateJWT(user *model.User) (string, time.Time, error) {
    return token.Sign(user.UserId)
}
```
